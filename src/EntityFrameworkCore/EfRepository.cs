...
namespace FuryTechs.BLM.EntityFrameworkCore
{
    ...
    where T : class, new()
  {
    ...
    /// </summary>
    public bool IgnoreLogicalDeleteError { get; set; }
    ...
    private IRepository GetChildRepositoryFor(EntityEntry entry)
    {
      ...
      var mi = GetType().GetMethods().FirstOrDefault(x => x.Name == "GetChildRepositoryFor" && x.IsGenericMethod);
      var genericMethod = mi?.MakeGenericMethod(repoType);

      return genericMethod?.Invoke(this, null) as IRepository;
      ;
    }
