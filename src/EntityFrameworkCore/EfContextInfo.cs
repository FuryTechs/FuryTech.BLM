...
namespace FuryTechs.BLM.EntityFrameworkCore
{
  public class EfContextInfo<TEntity> : IContextInfo
  {
    ...
    {
      return Authorize.Collection(_dbcontext.Set<T>(),
        new EfContextInfo<TEntity>(Identity, _dbcontext, _serviceProvider), _serviceProvider);
    }

    public async Task<IQueryable<T>> GetAuthorizedEntitySetAsync<T>() where T : class
    {
      return await Authorize.CollectionAsync(_dbcontext.Set<T>(),
        new EfContextInfo<TEntity>(Identity, _dbcontext, _serviceProvider), _serviceProvider);
    }
  }
}